...
.eseg
.org 0x000
smsc:	.db 11, 7, 9,1,6, 8,9,9, 9,1, 0,0	;телефон SMS сервис центра (1байт- кол-во знаков в номере)
.org 0x010
da:		.db 11, 7, 9,1,5, 9,6,3, 3,6, 8,5	;телефон получателя (1байт- кол-во знаков в номере)
.org 0x020
...
.def IDENT		= r8		;счетчик колличества запросов на посылку смс
...
;.def ERR_ST		= r9		;байт ответов на PDU
...
;Настроим АЦП
	...
	out ADMUX,r16
		...
		out TCNT1H,r10
		out TCNT1L,r10			;
		ldi r16,0b00000101		;разрешить прерывание по переполнению 1(16разр)и 0(8разр) счетчиков
		out TIMSK,r16
		ldi r16,0b00000101
		out TCCR1B,r16
		out TCCR0,r10
...
k300:
		sbic PINB,4			;кнопка 4	установка ключа охраны
		rjmp k400
		rcall key_busy
...
ret
; ----------------------------------------------
selecting_key:		;попытка подбора ключа
		push zh
		push zl
		push xh
		push xl
...
s_m5:	inc r17
		...
		cbr r16,0b00010000		;cнимаем флаг задачи
		mov SMS_ST,r16
...
id01:	lpm r19,z
		...
		rjmp id10
id02:	inc r17				;усли принятый байт совпал
		st x,r17
		cp r17,r18
		brne id10
		st x,r10
...
id11:	lpm r19,z
		...
		st x,r10			;обнуляем счетчик байтов этого слова
		rjmp id20
id12:	inc r17				;усли принятый байт совпал
		st x,r17
		cp r17,r18
		brne id20
		st x,r10
...
id21:	lpm r19,z
		...
		st x,r10			;обнуляем счетчик байтов этого слова
		rjmp id30
id22:	inc r17				;усли принятый байт совпал
		st x,r17
		cp r17,r18
		brne id30
		st x,r10
...
;		st x,r10			;обнуляем счетчик байтов этого слова
;		rjmp id40
;id32:	inc r17				;усли принятый байт совпал
;		st x,r17
;		cp r17,r18
;		brne id40
;		st x,r10
