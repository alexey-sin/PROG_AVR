...
volatile uint8_t MENU = 0;	//текущая позиция меню (индекс. от 0 до 9 включительно)
#define MUL_MENU_WAIT	5	//множитель на 2сек, ожидание выхода из меню
...
volatile uint8_t PWM1_LEVEL = 0;
volatile uint8_t PWM2_LEVEL = 0;
volatile uint8_t PWM3_LEVEL = 0;

volatile uint8_t M12_Step_Start = 0;	//счетчик разгона М1 и М2
//точки шагов
volatile uint8_t M1_begin_start_on_step = 0;
volatile uint8_t M1_end_start_on_step = 0;
volatile uint8_t M2_begin_start_on_step = 0;
volatile uint8_t M2_end_start_on_step = 0;

volatile uint8_t M12_Step_Stop = 0;		//счетчик торможения М1 и М2
volatile uint8_t M1_stop_on_step = 0;
//===============================================================================
// Вычисляемые константы
volatile uint8_t PWM1_Start_Discret = 0;	//значение PWM1 для старта в дискретах 
volatile uint8_t PWM2_Start_Discret = 0;	//значение PWM2 для старта в дискретах 
volatile uint8_t PWM3_Start_Discret = 0;	//значение PWM3 для старта в дискретах 
volatile double PWM1_Step_Discret = 0;	//сколько дискрет добавить к стартовому значению на 1 шаг разгона(секунду)
volatile double PWM2_Step_Discret = 0;	//сколько дискрет добавить к стартовому значению на 1 шаг разгона(секунду)
volatile double PWM3_Step_Discret = 0;	//сколько дискрет добавить к стартовому значению на 1 градус
//===============================================================================
...
/**
	...
	Вентиляторы: (питание всех 12 вольт, мощность 6-12Вт)
		...
		М2 => подача воздуха в камеру дожига пиролизных газов.
	...
	Индикация и управление:
		...
		Три кнопки: "М", "+", "-" 
			...
			Выход из режима меню - 5 секунд без нажатий на кнопки.
	...
	Работа автомата разбита условно на 2 независимых контура.
		...
		Процедура запуска вентияторов М1 и М2:
			разгон М1 от старта (уставка) до 100% в течении 30 секунд
			ожидание 30 секунд
		...
		P.S. В случае необходимости время разгонов вентиляторов и пауз ожидания можно тоже вынести в настройки.
*/
