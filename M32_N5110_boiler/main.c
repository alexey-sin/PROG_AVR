...
void Work_T12_M12(void){
	if(BitIsSet(STATUS, ST_M12_START)){	//разгон вентиляторов М1 и М2
		if(M12_Step_Start > M1_begin_start_on_step && M12_Step_Start <= M1_end_start_on_step){	//разгон М1
			PWM1_LEVEL = PWM1_Step_Discret * (M12_Step_Start - M1_begin_start_on_step) + PWM1_Start_Discret;
			PWM_send(PWM1, PWM1_LEVEL);
		}
		if(M12_Step_Start == (M1_end_start_on_step + 1)){	//последний шаг разгона М1
			PWM1_LEVEL = 255;
			PWM_send(PWM1, PWM1_LEVEL);
		}
		if(M12_Step_Start >= M2_begin_start_on_step && M12_Step_Start <= M2_end_start_on_step){	//разгон М2
			PWM2_LEVEL = PWM2_Step_Discret * (M12_Step_Start - M2_begin_start_on_step) + PWM2_Start_Discret;
			PWM_send(PWM2, PWM2_LEVEL);
		}
		if(M12_Step_Start == (M2_end_start_on_step + 1)){	//последний шаг разгона М2 и разгон М1 и М2 закончен
			PWM2_LEVEL = 255;
			PWM_send(PWM2, PWM2_LEVEL);
	...
	if(BitIsSet(STATUS, ST_M12_STOP)){	//остановка вентиляторов М1 и М2
		...
		if(M12_Step_Stop == M1_stop_on_step){
			PWM1_LEVEL = 0;
			PWM_send(PWM1, PWM1_LEVEL);
	...
	if(STATUS_DS == 1 && STATUS_TC1 == 1 && ((VALUE_TC1 - (uint8_t)DS_Temp) >= SETVAL_DELTA_TEMP_SHUTDOWN)){
		if(BitIsClr(STATUS, ST_WORK_T12_M12)) SetBit(STATUS, ST_WORK_T12_M12);
			...
			if(yesRoom && yesOven && PWM1_LEVEL == 0 && BitIsClr(STATUS, ST_M12_START)){
				M12_Step_Start = 0;
				ClrBit(STATUS, ST_M12_STOP);
				SetBit(STATUS, ST_M12_START);
			}
			//условия остановки вентиляторов 1 контура(М1 и М2)
			if((noRoom || noOven) && PWM1_LEVEL != 0 && BitIsClr(STATUS, ST_M12_STOP)){
				M12_Step_Stop = 0;
				ClrBit(STATUS, ST_M12_START);
				SetBit(STATUS, ST_M12_STOP);
			}
	}else{
		if(BitIsClr(STATUS, ST_M12_STOP) && PWM1_LEVEL != 0){
			M12_Step_Stop = 0;
			ClrBit(STATUS, ST_M12_START);
			SetBit(STATUS, ST_M12_STOP);
		}
...
void Work_T3_M3(void){
	if(STATUS_TC2 == 1 && VALUE_TC2 >= SETVAL_TEMP_PYROLYS_START){
		...
		if(VALUE_TC2 >= SETVAL_TEMP_PYROLYS_FULL) diskret = 255.0;
			...
			diskret = workTemp * PWM3_Step_Discret + PWM3_Start_Discret;
		}
		if(diskret > 255.0) level = 0xFF; else level = (uint8_t)diskret;
		if(PWM3_LEVEL != level){
			PWM3_LEVEL = level;
			PWM_send(PWM3, PWM3_LEVEL);
		}
	}else{
		if(PWM3_LEVEL != 0){
			PWM3_LEVEL = 0;
...
void CalculateCoefficients(void){
	...
	PWM1_Start_Discret = (uint8_t)dd;
	dd = (255.0 - PWM1_Start_Discret) / M1_START_PERIOD;
	PWM1_Step_Discret = (uint8_t)dd;

	dd = (255.0 * SETVAL_PWM_FAN2_START) / 100;
	PWM2_Start_Discret = (uint8_t)dd;
	dd = (255.0 - PWM2_Start_Discret) / M2_START_PERIOD;
	PWM2_Step_Discret = (uint8_t)dd;

	dd = (255.0 * SETVAL_PWM_FAN3_START) / 100;
	PWM3_Start_Discret = (uint8_t)dd;
	PWM3_Step_Discret = (255.0 - PWM3_Start_Discret) / (SETVAL_TEMP_PYROLYS_FULL - SETVAL_TEMP_PYROLYS_START);

	M1_begin_start_on_step = 0;
	M1_end_start_on_step = M1_begin_start_on_step + M1_START_PERIOD;
	M2_begin_start_on_step = M1_end_start_on_step + M12_BETWEEN_START_PERIOD;
	M2_end_start_on_step = M2_begin_start_on_step + M2_START_PERIOD;

	M1_stop_on_step = M12_BETWEEN_STOP_PERIOD;
}
...
void init(void){
    ...
    //1(16бит)таймер - основной тик ~2сек
...
void OutInfoToLCD(void){	//вывод оперативной инфомации на дисплей
	...
	OutUI8_6x8(CalculatePercent(PWM1_LEVEL));
	LCD_OutChar6x8('%');
	// LCD_OutChar6x8(' ');
	// OutUI8_6x8((uint8_t)DS_Temp);
	// LCD_OutChar6x8(' ');
	// OutUI8_6x8((SETVAL_TEMP_ROOM - SETVAL_DELTA_TEMP_ROOM));
	
	LCD_gotoXY(4, 0);	// row = 0-5(строки); col = 0-83(столбцы)
	LCD_OutString6x8((char*)LabelM2);
	OutUI8_6x8(CalculatePercent(PWM2_LEVEL));
	LCD_OutChar6x8('%');
	// LCD_OutChar6x8(' ');
	// OutUI8_6x8(VALUE_TC1);
	// LCD_OutChar6x8(' ');
	// OutUI8_6x8((SETVAL_TEMP_TOP_OVEN - SETVAL_DELTA_TEMP_TOP_OVEN));

	LCD_gotoXY(5, 0);	// row = 0-5(строки); col = 0-83(столбцы)
	LCD_OutString6x8((char*)LabelM3);
	OutUI8_6x8(CalculatePercent(PWM3_LEVEL));
	LCD_OutChar6x8('%');
	// LCD_OutChar6x8(' ');
	// OutUI8_6x8(yesRoom);
	// LCD_OutChar6x8(' ');
